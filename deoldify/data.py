...
class LabImageDataBunch(DataBunch):
    ...
    def create_from_ll(cls, lls:LabelLists, bs:int=64, val_bs:int=None, ds_tfms:Optional[TfmList]=None,
                ...
                mode:str='bilinear', tfm_y:bool=False)->'LabImageDataBunch':
        "Create an `LabImageDataBunch` from `LabelLists` `lls` with potential `ds_tfms`."
    ...
    def from_folder(cls, path:PathOrStr, train:PathOrStr='train', valid:PathOrStr='valid',
                    valid_pct=None, classes:Collection=None, **kwargs:Any):
        "Create from imagenet style dataset in `path` with `train`,`valid`,`test` subfolders (or provide `valid_pct`)."
        path=Path(path)
        il = LabLabImageList.from_folder(path)
...
class LabImage(Image):
    ...
    def __init__(self, px:Tensor,l,ab):
        self._px = px
        self._logit_px=None
        self._flow=None
    ...
    def clone(self):
        ...
        return self.__class__(self.px.clone(), self.l, self.ab)

    def __repr__(self): 
        return f'L= {self.l.shape}, ab= {self.ab.shape}'
...
class LabLabelList(LabelList):

    def __init__(self, ll:LabelList):
        self.x,self.y,self.tfm_y = ll.x,ll.y,ll.tfm_y
        self.y.x = ll.x
        self.item=None
        #self.transform(ll.tfms, **kwargs)

    def new(self, x, y, **kwargs)->'LabLabelList':
        if isinstance(x, ItemList):
            return self.__class__(LabelList(x, y, tfms=self.tfms, tfm_y=self.tfm_y, **self.tfmargs))

    def __getitem__(self,idxs:Union[int,np.ndarray])->'LabLabelList':
        idxs = try_int(idxs)
...
class LabImageList(ItemList):
    "`ItemList` suitable for computer vision."
    _bunch,_square_show,_square_show_res = LabImageDataBunch,True,True
    ...
    def open(self, fn):
        ...
        return open_lab_image(fn, convert_mode=self.convert_mode, after_open=self.after_open)
    ...
    def label_from_func(self, func:Callable, label_cls:Callable=None, **kwargs):#->'LabLabelList':
        "Apply `func` to every input to get its label."
        return LabLabelList(self._label_from_list([func(o) for o in self.items], label_cls=label_cls, **kwargs))

    def label_from_folder(self, label_cls:Callable=None, **kwargs):#->'LabLabelList':
        "Give a label to each filename depending on its folder."
        return LabLabelList(self.label_from_func(func=lambda o: (o.parts if isinstance(o, Path) else o.split(os.path.sep))[-2],label_cls=label_cls, **kwargs))
...
class LabLabImageList(LabImageList):
    _label_cls = LabImageList
...
class ObjectItemList(LabImageList):
...
class SegmentationLabelList(LabImageList):
...
class SegmentationItemList(LabImageList):
...
class PointsItemList(LabImageList):
    "`ItemList` for `Image` to `ImagePoints` tasks."
    _label_cls,_square_show_res = PointsLabelList,False

class ImageImageList(LabImageList):
    "`ItemList` suitable for `Image` to `Image` tasks."
    _label_cls,_square_show,_square_show_res = LabImageList,False,False
